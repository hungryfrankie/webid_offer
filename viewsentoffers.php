...
if (!$user->checkAuth()) {
    ...
    $_SESSION['REDIRECT_AFTER_LOGIN'] = 'youroffers.php';
...
$query = "SELECT count(*) As COUNT FROM " . $DBPrefix . "offers WHERE offer_sender=:offer_sender";
$params = array();
$params[] = array(':offer_item_owner', $user->user_data['id'], 'int');
$params[] = array(':offer_sender', $user->user_data['id'], 'int');
...
$query = "SELECT * FROM " . $DBPrefix . "offers 
          WHERE offer_sender = :offer_sender
...
while ($row = $db->fetch()) {
    ...
    if ($row['offer_status']=='pending'){
        ...
        $b_pending_class='btn btn-default form-group';
    }
    ...
    if ($row['offer_status']=='declined'){
        ...
        $b_counter_offer_class='hidden'; 
        $b_accept_class='hidden';
        $b_rejected_class='btn btn-warning form-group';
        $b_expired_class='hidden';
        $b_pending_class='hidden';
    }
    if ($row['offer_status']=='accepted'){
        if ($row['offer_item_owner'] == $user->user_data['id'])
        {   
            $b_accept_class='btn btn-primary form-group';   
            $b_buy_now_class='hidden';
        } else{
            $b_accept_class='hidden';
    ...
    if ($row['offer_status']=='counter_offer'){
         $b_buy_now_class='hidden';
         $b_reject_class='hidden';
         $b_counter_offer_class='btn btn-info form-group';   
         $b_accept_class='hidden';
         $b_rejected_class='hidden';
         $b_expired_class='hidden';
         $b_pending_class='hidden';
    ...
    $template->assign_block_vars('offers', array(
            ...
            'OFFER_ITEM_OWNER' => $row['offer_iitem_owner'],
            ...
            'B_PENDING_CLASS' =>$b_pending_class
            ));
...
if ($PAGES > 1) {
    ...
    while ($COUNTER <= $PAGES && $COUNTER < ($PAGE + 6)) {
        // if ($COUNTER==$PAGE) $PAGE = '<u> '.$COUNTER.'</u>';
         $template->assign_block_vars('pages', array(
                 'PAGE' => ($PAGE == $COUNTER) ?  '<a  style="background-color:lightblue;opacity: 0.3;"><u>'.$COUNTER . '</u></a>' : '<a href="' . $system->SETTINGS['siteurl'] . 'viewsentoffers.php?PAGE=' . $COUNTER . '"><u>' . $COUNTER . '</u></a>'
                 ));
         $COUNTER++;
     }
 ...
 $template->assign_vars(array(
         ...
         'PREV' => ($PAGES > 1 && $PAGE > 1) ? '<a href="' . $system->SETTINGS['siteurl'] . 'viewsentoffers.php?PAGE=' . $PREV . '"><u>' . $MSG['5119'] . '</u></a>&nbsp;&nbsp;' : '',
         'NEXT' => ($PAGE < $PAGES) ? '<a href="' . $system->SETTINGS['siteurl'] . 'viewsentoffers.php?PAGE=' . $NEXT . '"><u>' . $MSG['5120'] . '</u></a>' : '',
         'PAGE' => $PAGE,
         'PAGES' => $PAGES,
...
include 'header.php';
$TMP_usmenutitle = $MSG['9902'];
include INCLUDE_PATH . 'user_cp.php';
$template->set_filenames(array(
    'body' => 'yoursentoffers.tpl'
        ));
