...
//$id = intval($_REQUEST['id']);
  ...
  $price = (isset($_POST['offer_item_price'])) ? $_POST['offer_item_price'] : '';
  $offer_id= (isset($_POST['offer_id'])) ? $_POST['offer_id'] : '';
  $quantity = (isset($_POST['offer_item_qty'])) ? $_POST['offer_item_qty'] : '1';
if (!$user->checkAuth()) {
    $_SESSION['LOGIN_MESSAGE'] = $MSG['5002'];
    $_SESSION['REDIRECT_AFTER_LOGIN'] = 'pay_now.php?id=' . $id;
...
// finding values from offer
$query = "SELECT * FROM " . $DBPrefix . "offers WHERE offer_id = :offer_id";
$params = array();
$params[] = array(':offer_id', $offer_id, 'int');
$db->query($query, $params);
while ($row = $db->fetch()) {
// if offer is less than the auto reject this should nver happen
if ($price <= $Auction['make_offer_reject']) {
    $template->assign_vars(array(
            'TITLE_MESSAGE' => $MSG['9415'],
            'BODY_MESSAGE' => $ERR_9606
            ));
    include 'header.php';
    $template->set_filenames(array(
            'body' => 'message.tpl'
...
// if offerprice is changed from what is in the db 

if ($row['offer_price'] != $price) {
    $template->assign_vars(array(
            'TITLE_MESSAGE' => $MSG['9416'],
            'BODY_MESSAGE' => $ERR_9607
            ));
    include 'header.php';
    $template->set_filenames(array(
            'body' => 'message.tpl'
...
if ($row['offer_status'] != 'accepted') {
    $template->assign_vars(array(
            'TITLE_MESSAGE' => $MSG['9417'],
            'BODY_MESSAGE' => $ERR_9608
            ));
    include 'header.php';
    $template->set_filenames(array(
            'body' => 'message.tpl'
...
if ($Auction['bn_only'] == 0) {
    if (!( $price  > 0 && ($Auction['num_bids'] == 0 || ($Auction['reserve_price'] > 0 && $Auction['current_bid'] < $Auction['reserve_price']) || ($Auction['current_bid'] <  $price )))) {
        $ERR = $ERR_712;
    } else {
...
if (isset($_POST['action']) && $_POST['action'] == 'buy') {
    ...
    if (!isset($ERR)) {
        ...
        $params[] = array(':buy_now',  $price , 'float');
        ...
        if ($Auction['bn_only'] == 0) {
            ...
            if (($quantity - $qty) == 0) {
                ...
                $params = array();
                $params[] = array(':auc_id', $id, 'int');
                $params[] = array(':current_bid',  $price , 'int');
                $params[] = array(':current_bid_id', $current_bid_id, 'int');
                $db->query($query, $params);
            }
            ...
            if ($system->SETTINGS['fees'] == 'y' && !$user->permissions['no_fees']) {
                ...
                if ($row['fee_type'] == 'flat') {
                    ...
                    $fee_value = ($row['value'] / 100) * floatval( $price ) * $qty;
                }
                ...
                $fee_value = 0;
                while ($row = $db->fetch()) {
                    if (floatval( $price ) >= $row['fee_from'] && floatval( $price ) <= $row['fee_to']) {
                        if ($row['fee_type'] == 'flat') {
                            $fee_value = $row['value'] * $qty;
                        } else {
                            $fee_value = ($row['value'] / 100) * floatval( $price ) * $qty;
            ...
            if ($Auction['initial_quantity'] == 1 || ($quantity - $qty) == 0) {
                $tmpauc = $Auction;
                include 'cron.php';
                $Auction = $tmpauc;
                unset($tmpauc);
...
$BN_total = ( $price  * $qty) + $shipping_cost;
...
$template->assign_vars(array(
        ...
        'TITLE' => htmlspecialchars($Auction['title']),
        'BN_PRICE' => $system->print_money($price),
