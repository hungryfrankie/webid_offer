...
$query = "SELECT count(*) As COUNT FROM " . $DBPrefix . "offers WHERE offer_reciever=:offer_reciever and  offer_item_owner != :offer_item_owner";
$params = array();
...
while ($row = $db->fetch()) {
   // for ($x = 0; $x <= $total+1; $x++) {
    ...
    if ($row['offer_status']=='counter_offer'){
         ...
         $b_rejected_class='hidden';
         $b_expired_class='hidden';
         $b_counter_class='btn btn-danger form-group';
...
if ($PAGES > 1) {
    ...
    while ($COUNTER <= $PAGES && $COUNTER < ($PAGE + 6)) {
        $template->assign_block_vars('pages', array(
                'PAGE' => ($PAGE == $COUNTER) ? '<ul class="pagination"><li >' . $COUNTER . '</li></ul>' : '<a href="' . $system->SETTINGS['siteurl'] . 'youroffers.php?PAGE=' . $COUNTER . '"><u class="pagination">' . $COUNTER . '</u></a>'
                ));
        $COUNTER++;
    }
}
...
$template->assign_vars(array(
        ...
        'GOTO_PAGE' =>'youroffers.php',
        'PREV' => ($PAGES > 1 && $PAGE > 1) ? '<a href="' . $system->SETTINGS['siteurl'] . 'youroffers.php?PAGE=' . $PREV . '"><u>' . $MSG['5119'] . '</u></a>&nbsp;&nbsp;' : '',
        'NEXT' => ($PAGE < $PAGES) ? '<a href="' . $system->SETTINGS['siteurl'] . 'youroffers.php?PAGE=' . $NEXT . '"><u>' . $MSG['5120'] . '</u></a>' : '',
        'PAGE' => $PAGE,
        'PAGES' => $PAGES,
